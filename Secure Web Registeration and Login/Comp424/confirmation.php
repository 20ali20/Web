...
if($result){
    ...
    if($countkeys == 1){
        ...
        $securityA=trim($rows['securityAnswer']);
        $securityQ=trim($rows['securityQuestion']);
        
        
        //set num of login to 0 since user has not logged in yet
        $numLogin= (int)0;
        
        //set to Not logged in yet   since user has not logged in yet
        $lastLogin = "NA";
        ...
        $query2 = "INSERT INTO users (username,password,firstName,lastName,birthday,email,securityQuestion,securityAnswer,salt,lastLogin,numLogin)VALUES('$uname','$hashedPass','$fname','$lname','$bDay','$email','$securityQ','$securityA','$salt','$lastLogin','$numLogin')";
        $infinaltable = mysqli_query($connection,$query2) or trigger_error("ERROR: ".mysqli_error(),E_USER_ERROR);
